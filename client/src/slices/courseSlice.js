  ...
  async (courseId, thunkAPI) => {
    try {
      ...
      return thunkAPI.rejectWithValue(
        e.response?.data?.message || "Enroll failed"
      );
    }
  }
);

export const myEnrollments = createAsyncThunk(
...
export const markProgress = createAsyncThunk(
  "courses/markProgress",
  async ({ enrollmentId, contentId }, thunkAPI) => {
    try {
...
const courseSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(myEnrollments.fulfilled, (s, a) => {
        s.enrollments = a.payload;
        console.log("✅ Enrollments synced globally");
      })

      .addCase(markProgress.fulfilled, (s, a) => {
        const idx = s.enrollments.findIndex((e) => e.id === a.payload._id);
        if (idx >= 0) {
          s.enrollments[idx].progress = a.payload.progress;
          s.enrollments[idx].completedContentIds =
            a.payload.completedContentIds;
        }
      });
  },
});
